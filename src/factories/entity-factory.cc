...
Entity EntityFactory::CreateWarrior(Entity* parent) {
  ...
  return Entity(EntityKind::Warrior,
                parent,
                {ability_die,
                 ability_kick,
                 ability_pick,
...
Entity EntityFactory::CreateChest(Entity* parent, int init_lock_lvl,
  ...
  return Entity(EntityKind::Chest,
                parent,
                {ability_die, ability_contain, ability_is_transparent},
                subentites);
}
...
Entity EntityFactory::CreateTile(Entity* parent, int pos_x, int pos_y, int walkability_level) {
    ...
    return Entity(EntityKind::Tile, parent, {ability_contain, ability_is_transparent,
ability_is_walkable, ability_is_positioned});
}

Entity EntityFactory::CreateMap(int width, int height) { // FIXME поверхностное копирование, дед на
прошлом занятии говорил Entity world_map = _CreateMapShell(width, height);
...
Entity EntityFactory::_CreateMapShell(int width, int height) {
    Ability ability_be_map  = Ability(AbilityKind::IsMap, {{AbilityState::MapWidth, width},
{AbilityState::MapHeight, height}}); Ability ability_contain = Ability(AbilityKind::CanContain,
{{AbilityState::ContainCapacity, width * height + 1}});
...
void EntityFactory::_InitMap(Entity& map_shell, int width, int height) {
...
// Entity& EntityFactory::CreateMap(int width, int height) {
//     Ability ability_contain = AbilityFactory::CreateAbilityContain(width * height);
