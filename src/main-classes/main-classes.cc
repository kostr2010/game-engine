...
int Ability::GetStateValue(const AbilityState& state_name) { // operator[]  conflicts with const
                                                             // because returns lvalue? aka type&
...
std::ostream& operator<<(std::ostream& stream, Ability& ability) {
  ...
  case AbilityKind::CanContain:
    stream << "CanContain: ContainCapacity - "
           << ability.GetStateValue(AbilityState::ContainCapacity) << "\n";
    break;
  ...
  case AbilityKind::IsMap:
    stream << "IsMap: MapWidth - " << ability.GetStateValue(AbilityState::MapWidth)
           << ", MapHeight - " << ability.GetStateValue(AbilityState::MapHeight) << "\n";
    break;
  ...
  case AbilityKind::IsPositioned:
    stream << "IsPositioned: PositionX - " << ability.GetStateValue(AbilityState::PositionX)
           << ", PositionY - " << ability.GetStateValue(AbilityState::PositionY) << "\n";
    break;
...
Entity::Entity(const EntityKind& kind, Entity* parent,
...
void Entity::AddSubentity(const std::vector<Entity>& items) {
  ...
  for (const Entity& item : items) {
    bool can_fit = subentities_.size() <
                   abilities_[AbilityKind::CanContain].GetStateValue(AbilityState::ContainCapacity);
...
void Entity::RemoveSubentity(int index) {
  if (abilities_.count(AbilityKind::CanContain) == 0)
    return;
...
Entity* Entity::GetSubentity(int index) {
...
int Entity::CheckIfInRange(const Entity&      target,
                           const AbilityKind& ability) { // operator[] vs const method
  ...
  int pos_x =
      parent_tile->abilities_[AbilityKind::IsPositioned].GetStateValue(AbilityState::PositionX);
  int pos_y =
      parent_tile->abilities_[AbilityKind::IsPositioned].GetStateValue(AbilityState::PositionY);

  int range = abilities_[ability].GetStateValue(AbilityState::Range);

  // index of (x, y) = y * width + x
