...
class Entity {
public:
  Entity(const EntityKind& kind, Entity* parent, const std::vector<Ability>& abilities = {},
  ...
  void          RemoveSubentity(const int index);
  Entity*       GetSubentity(const int index);    // redo as Entity&
  Entity*       GetParentTile() const;            // redo as Entity&
  int           CheckIfHas(const Entity& target); // -1 if not found, else index
  int           CheckIfInRange(const Entity&      target,
                               const AbilityKind& ability); // -1 if not in range, else index of tile
