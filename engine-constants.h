...
std::map<AbilityKind, std::function<void(Entity &origin, Entity &target)>> dict_ability_dispatcher =
    {
        ...
        {CanPick,
         [](Entity &origin, Entity &target) -> void {
             ...
             bool target_can_be_picked =
                 origin_can_contain && target.abilities_.count(CanBePicked) != 0;
             bool origin_can_fit = target_can_be_picked &&
                                   origin.InventoryGetSize() <
                                       origin.abilities_[CanContain].GetStateValue(ContainCapacity);
        ...
        {CanLoot,
         [](Entity &origin, Entity &target) -> void {
             ...
             if (target_can_be_looted && state_locked == 0 &&
                 spot_to_loot < target.InventoryGetSize()) {
